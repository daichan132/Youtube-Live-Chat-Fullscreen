...
The panel appears only in full screen, only on a watch page with chat — on an ordinary video, or a stream whose replay is gone, the button never shows. In windowed and theater mode the chat is already beside the player. This Chrome build also runs in Opera and other Chromium browsers; Firefox has its own build on addons.mozilla.org.
