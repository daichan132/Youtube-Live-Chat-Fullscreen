...
This extension puts it back over the video, so you can read every message, reply, and send Super Chats on a live stream — without ever leaving full screen. It starts as a solid white panel with black text; color, transparency and text size are yours to change.

No shrinking the player, no theater mode, no popped-out window — the conversation is simply there, and one button in the control bar turns it off and on. Free, open source, desktop browsers only.

NOT A LOOKALIKE — YOUTUBE'S OWN CHAT, ON THE VIDEO
The panel over the video is YouTube's own live chat, the one from the sidebar. Member badges, emoji, stickers, the message box, the Super Chat and Super Sticker purchase panel — all of it YouTube's own. Posting goes through that box, so you must be signed in, as always. Scrolling back and members-only chat work as they do beside the player. The extension changes the look: colors, sizes, spacing.
...
Seven presets are built in, one click each. Pick Large Text — 18px black type on an almost-solid white panel, no usernames or profile pictures — and the chat reads from the sofa. Save your own, then rename, reorder or delete them.

WHAT YOU CAN CONTROL
Position: put the panel anywhere over the player. Hover over it for a control row — settings button and drag handle — just below, bottom right (low on the screen it rides up over the panel). With the row up, Tab to the handle and arrow keys nudge it 10px. It hops once to the clearest corner when captions, menus, the end screen or the controls start to cover it. Place it yourself and it stays put.
...
Background: color and transparency from one picker, plus 0-20px blur for a frosted look over busy footage. Blur starts at 0 — the one setting that can slow an older machine, so leave it at 0 there.
Text: 10px to 40px, 13px by default — 40px is about three times the size of normal chat text. Resizing the panel never changes the size you picked. Set the text color, a separate one for member names, and one of 50 fonts, Japanese ones included; each applies as you pick it.
Spacing: 0-40px between messages, plus switches for the Super Chat ticker bar and for usernames and profile pictures on ordinary messages (paid ones keep theirs).
...
Two permissions: activeTab and storage. The only web page it runs in is www.youtube.com — that is all Chrome's install prompt means when it mentions reading and changing your data. Nothing runs in the background: with no YouTube tab open, only an open popup does anything. All the code ships inside the extension; none of it is fetched later. Fonts are the exception: the default loads nothing; any other, or a preset using one, comes from Google Fonts.
No analytics, no tracking, nothing sent to the developer: the extension never collects, stores or sends what you read and type. Super Chat payments go through YouTube's purchase panel, not the extension; the one control it changes there is Membership, which opens the channel's join page in a new tab. Settings stay in this browser on this computer, never synced through your Google account; the popup exports them to a file you can import elsewhere.
...
The panel appears only in full screen, only on a watch page with chat — on an ordinary video, or a stream whose replay is gone, the button never shows. Outside full screen, YouTube shows the chat beside the player. This Chrome build also runs in Opera; Firefox has its own build on addons.mozilla.org.

KEYBOARD ACCESS AND TROUBLESHOOTING
Every setting is labeled for a screen reader, and sliders and pickers take arrow keys; only resizing and bringing up the control row need a mouse. Messages are YouTube's own, so a screen reader reads them as it does the sidebar. Automated WCAG 2.1 AA checks run on every code change, covering the popup, control row and settings panel.
