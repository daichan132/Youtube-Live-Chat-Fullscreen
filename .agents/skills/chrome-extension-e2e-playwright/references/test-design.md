...
### ワーカー間 URL 共有（並列実行の最適化）

Worker-scoped fixture で動的 URL 探索を行う場合、各ワーカーが独立に同じ探索を繰り返すのは無駄が大きい。ファイルベースのキャッシュ（`$TMPDIR` に plain text）で発見済み URL をワーカー間で共有するパターンが有効:

- **global-setup**: キャッシュファイルを削除（各テストランはクリーンスタート）
- **探索関数の先頭**: キャッシュを読み、有効性を検証して即 return
- **URL 発見時**: キャッシュに書き込み（他ワーカーが再利用）

読み書きは best-effort（`try/catch` で握りつぶし）で十分。ファイルの原子性は不要 — 最悪でも探索のフォールバックがある。
