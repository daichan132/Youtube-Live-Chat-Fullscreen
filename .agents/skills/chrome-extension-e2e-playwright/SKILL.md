...
Chrome 拡張は `chromium.launchPersistentContext` でしかロードできない（[公式ガイド](https://playwright.dev/docs/chrome-extensions)）。通常の `browser.newContext()` は使えない。`headless: false` が安定だが、CI では `channel: 'chromium'`（New Headless）で headless 実行も可能。`xvfb-run` で仮想ディスプレイを提供する方法もある。global setup でビルド出力の存在確認をしておくとよい。
