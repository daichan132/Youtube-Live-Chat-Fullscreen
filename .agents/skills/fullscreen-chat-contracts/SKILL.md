...
live / archive / no-chat 境界の破壊を防ぎ、過去の回帰バグを再発させないためのガイド。実ブラウザでの確認は `ylc-agent-browser` を前提にする。
...
YouTube はフルスクリーン進入時に `#chat-container` を `#secondary-inner` から `#panels-full-bleed-container` に移動する。この挙動はコードからは読み取れず、`ylc-agent-browser` / `agent-browser` で実際に確認する必要がある。
