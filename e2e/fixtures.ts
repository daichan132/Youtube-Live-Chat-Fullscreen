...
>({
  ...
  page: async ({ sharedPage, sharedExtension }, use, testInfo) => {
    ...
    // An application runtime may write back stale state via storage.onChanged.
    ...
    await sharedExtension.storage.clear().catch(() => null)
    // Keep popup assertions deterministic while preserving the runtime's
    // browser-locale fallback for real users.
    await sharedExtension.storage
      .set({ [LOCALE_STORAGE_KEY]: { schemaVersion: 1, writerId: 'e2e-fixture', value: 'en' } })
      .catch(() => null)
