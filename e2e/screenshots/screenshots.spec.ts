...
test.describe
  .serial('Screenshots', () => {
    ...
    test('Popup screenshots (light & dark)', async ({ page, extension }) => {
      ...
      // Dark theme popup
      await setTheme(page, extension, 'dark')
      await popupRoot.screenshot({ path: screenshotPath('popup-dark') })

      // Restore light theme
      await setTheme(page, extension, 'light')
    })

    test('Fullscreen chat light + settings', async ({ page, extension }) => {
      test.setTimeout(300000)

      // Seed store and patch overlay settings
      const patched = await patchOverlayStore(extension, OVERLAY_OVERRIDES)
    ...
    test('Dark theme (chat, controls & settings)', async ({ page, extension }) => {
      test.setTimeout(300000)
