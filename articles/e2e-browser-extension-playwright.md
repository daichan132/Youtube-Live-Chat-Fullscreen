...
export const test = base.extend<{ context: BrowserContext; extensionId: string }>({
  context: async ({}, use) => {
    ...
    try {
      context = await chromium.launchPersistentContext(userDataDir, {
        // New Headless で拡張を動かすには channel: 'chromium' が必要（後述）。headed が安定
        headless: false,
  ...
  extensionId: async ({ context }, use) => {
    ...
    // 起動済みの Worker を探し、なければ waitForEvent で待つ
    let worker = context.serviceWorkers().find(isExtensionWorker)
    if (!worker) {
      // waitForEvent は新たに起動した Worker のみキャプチャする
      worker = await context.waitForEvent('serviceworker', {
        predicate: isExtensionWorker,
        timeout: 30_000,
      })
    }
    // chrome-extension://<id>/service-worker.js → host 部分が Extension ID
...
async function waitForMv3Worker(context: BrowserContext) {
  const findWorker = () =>
...
```typescript
async function waitForMv3WorkerWithCdpFallback(context: BrowserContext) {
  const findWorker = () =>
    context.serviceWorkers().find(isExtensionWorker) ?? null
  ...
  try {
    ...
    const retryWorkerPromise = context.waitForEvent('serviceworker', {
      predicate: isExtensionWorker,
      timeout: 10_000,
    }).catch(() => null)

    const retryWarmup = await context.newPage()
    await retryWarmup.goto('https://www.youtube.com', {
      waitUntil: 'domcontentloaded',
      timeout: 15_000,
    }).catch(() => {})

    worker = findWorker() ?? await retryWorkerPromise
    await retryWarmup.close().catch(() => {})
  } catch {
...
### 設計方針: まずは Worker 固定、必要ならフォールバック
...
const data = await worker.evaluate(
  async (keys) => chrome.storage.local.get(keys), key
)
...
```typescript
/** React-free な bridge ページ（e2e.html）で storage 操作を実行する */
...
function createStorageAccessor(
  ...
  const withFallback = async <T>(
    viaWorker: (w: Worker) => Promise<T>,
    viaPage: (page: Page) => Promise<T>,
  ): Promise<T> => {
    if (worker) {
      try {
  ...
  return {
    get: (keys) => withFallback(
      ...
      (page) => page.evaluate((keys) => chrome.storage.local.get(keys), keys ?? null),
    ),
    set: (items) => withFallback(
      (w) => w.evaluate((items) => chrome.storage.local.set(items), items).then(() => {}),
      (page) => page.evaluate((items) => chrome.storage.local.set(items), items).then(() => {}),
    ),
    clear: () => withFallback(
      (w) => w.evaluate(() => chrome.storage.local.clear()).then(() => {}),
      (page) => page.evaluate(() => chrome.storage.local.clear()).then(() => {}),
    ),
  }
}
```
...
`force: true` や JS クリックに逃げる前に、**オーバーレイ自体を除去するのが安定化の近道**です。次セクション（パターン 4）で説明する `addLocatorHandler()` で同意ダイアログ等を自動処理すれば、大半の「クリックが効かない」はそもそも発生しなくなります。以下の段階的クリックは、`addLocatorHandler()` を設定した上でまだ actionability が揺れる要素向けのパターンです。

### 解決: 段階的クリック（デフォルト2段階 + 任意3段階目）

まず通常クリック、次に `force: true` を試し、それでも安定しない要素だけ JS フォールバックを明示 opt-in で有効化します。
 ...
 * Stage 1 の通常クリックが重要: force:true は actionability checks をスキップするため
 * addLocatorHandler()（例: 同意ダイアログの自動処理）が発火しない。
 */
async function reliableClick(
  ...
  const allowJsFallback = options.allowJsFallback ?? false
  ...
  try {
    ...
    // actionability failure — escalate to next stage
  }
  ...
  try {
    ...
    // click dispatch failure — escalate to next stage
  }
  if (await verify().catch(() => false)) return
...
ポイントは **「常に 3 段階を踏まない」** ことです。`force: true` までで安定するならそれで止め、JS フォールバックは限定的に使います。
...
await expect.poll(async () => {
  return page.evaluate(() => {
    const shadowRoot = document.querySelector('#my-ext')?.shadowRoot
...
await expect.poll(async () => {
  return page.evaluate(() => {
    const shadowRoot = document.querySelector('#my-ext')?.shadowRoot
    return shadowRoot?.querySelector('[data-ext-state="ready"]') !== null
  })
}).toBe(true)
```
...
async function captureDiagnosticState(page: Page, testInfo: TestInfo, label: string) {
  try {
    const state = await page.evaluate(() => ({
      ...
      shadowContent: (() => {
        const shadowRoot = document.querySelector('#my-ext-root')?.shadowRoot
        return {
          exists: shadowRoot !== null,
          childCount: shadowRoot?.children.length ?? 0,
          iframeCount: shadowRoot?.querySelectorAll('iframe').length ?? 0,
        }
      })(),
    }))
...
- [Playwright - Best Practices](https://playwright.dev/docs/best-practices) — 外部依存テストの切り分け方針（実サービスは少数スモーク）
