...
//
// The summary must name posting an ordinary comment AND sending a Super Chat as
// distinct capabilities. Both have been silently lost to compression before,
// leaving a summary that reads as if only the paid action were possible. Only
// the reference locales can be checked mechanically -- the other 53 express it
// in their own words -- so treat the reference check as a canary and re-read
// every locale's summary by hand whenever this line changes.
//
...
// Reference-locale canary for the capability pair described above.
const SUMMARY_CAPABILITIES = {
  en: [/\bpost\b|\bcomments?\b/i, /super chat/i],
  ja: [/コメント/, /スーパーチャット|スパチャ/],
}
...
for (const code of localeCodes) {
  ...
  for (const pattern of SUMMARY_CAPABILITIES[code] ?? []) {
    if (!pattern.test(summary)) {
      fail(`${code}: extensionDescription no longer matches ${pattern} — the summary must name both posting a comment and sending a Super Chat`)
    }
  }
